...
public class AuthService : Service<ApplicationUser>, IAuthService
{
    ...
    public async Task<ApiResponse> Register(CreateAppUserDto request)
    {
        try
        {
            var isPhoneUnique = await IsPhoneNumberUnique(request.PhoneNumber);
            var isEmailUnique = await IsEmailUnique(request.Email);
            var isUserUnique = await IsUserUnique(request.FullName.ToLower());
            if (!isPhoneUnique)
            {
                _response.Success = false;
                _response.StatusCode = HttpStatusCode.BadRequest;
                _response.Message = "Phone number is already in use.";
                return _response;
            }
            ...
            _response.Result = isPhoneUnique;
            return _response;
